"""
serialisation function to support JsonBlockField field
"""
...
class BasicSerial(object):
    ...
    convert = [
                   [datetime.datetime,
                   {
                    ...
                    "restore": lambda x: datetime.datetime.fromtimestamp(x),
                    }],
                   ]
    ...
    def convert_object(cls, obj):
        ...
        for a in cls.convert:
            if isinstance(obj, a[0]):
                return {"_type": a[0].__name__,
                        "_content": a[1]["convert"](obj)}            
        ...
        if isinstance(obj, dict):
            ...
            try:
                ...
                try:
                    ...
                    """
                    process the harder ones
                    """
                    for a in cls.convert:
                        if obj["_type"] == a[0]:
                            return a[1]["restore"](obj)  
